...
function checkOverriddenStoreValue(obj, selector) {
    ...
    if (!$(obj).is(':checked')) {
        ...
        $.each(elementsArray, function (key, value) {
            var kenoduiElement = $(value).data("kendoNumericTextBox");
            if (kenoduiElement !== undefined && kenodui !== null) {
...
function StartPageLoading(options) {
    if (options && options.animate) {
        ...
        $('.page-loading').remove();
        $('body').append('<div class="page-loading"><img src="/assets/administration/build/images/loading.gif"/>&nbsp;&nbsp;<span>' + (options && options.message ? options.message : 'Loading...') + '</span></div>');
    }
}
