                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 col-xs-12 text-right">
                                    <div class="all-vol-btn">
                                        <a href="{{ route('admin.products.sizes.addEdit') }}">
                                            <button>
                                    ...
                                    <tbody>
                                        @if(count($lists) > 0)
                                            @foreach($lists as $list)
                                                <tr>
                                                    <td>{{ ucfirst($list['size_name']) }}</td>
                                                    <td>
                                                        {{ ucfirst($list['set_order']) }}
                                                    </td>
                                                    <td>
                                                        @if($list['status'] == 1)
                                                            <a href="{{ route('admin.products.sizes.toggleStatus', [$list['id'], 0]) }}">
                                                                <button type="submit" class="custon-active">Active</button>
                                                            </a>
                                                        @else
                                                            <a href="{{ route('admin.products.sizes.toggleStatus', [$list['id'], 1]) }}">
                                                                <button type="submit" class="custon-delete">Inactive</button>
                                                            </a>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <a href="{{ route('admin.products.sizes.addEdit', [$list['id']]) }}"
                                                            style="color:green;padding: 5px;" title="edit">
                                                            <i class="fa far fa-edit fa-lg"></i>
                                                        </a>

                                                        <a href="{{ route('admin.products.sizes.delete', [$list['id']]) }}"
