<div class="inner-content-area">
  <div class="row justify-content-center">
    <div class="col-md-12">
      ...
      <form method="POST" action="{{ url('admin/Products/AutoSizeAdd') }}" enctype="multipart/form-data" class="form-horizontal" id="auto_size_add_form">
        @csrf
                  ...
                  <select name="size_id" class="form-control" required>
                    ...
                    @if(isset($sizes))
                      @foreach($sizes as $size)
                        <option value="{{ $size->id }}" {{ ($size->id == ($size_id ?? '')) ? 'selected="selected"' : '' }}>
                          {{ $size->size_name }}
                        </option>
                      @endforeach
                    @endif
...
$('#auto_size_add_form').validate({
  ...
  submitHandler: function(form) {
    ...
    $.ajax({
      type: "POST",
      url: '{{ url("admin/Products/AutoSizeAdd") }}',
      data: $(form).serialize(),
      beforeSend: function() {
