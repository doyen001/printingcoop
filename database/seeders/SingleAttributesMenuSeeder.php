...
class SingleAttributesMenuSeeder extends Seeder
{
    /**
     * Run database seeds.
     */
    public function run(): void
    {
        ...
        $existingSubModule = DB::table('sub_modules')
        ...
        foreach ($adminUsers as $admin) {
            ...
            // Assign the single attributes sub-module to all admin users
            $singleAttributesSubModule = DB::table('sub_modules')
                ->where('module_id', $productModuleId)
                ->where('url', 'singleAttributes')
                ->first();

            if ($singleAttributesSubModule) {
                $existingSubAssignment = DB::table('admin_sub_modules')
                    ->where('admin_id', $admin->id)
                    ->where('module_id', $productModuleId)
                    ->where('sub_module_id', $singleAttributesSubModule->id)
                    ->first();
                ...
                if (!$existingSubAssignment) {
                    DB::table('admin_sub_modules')->insert([
                        'admin_id' => $admin->id,
                        'module_id' => $productModuleId,
                        'sub_module_id' => $singleAttributesSubModule->id,
                    ]);
                }
            }
        }
